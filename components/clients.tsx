...
const accreditations = [
  { name: "ISO 9001:2015 Certified", image: "/iso-9001-certified.svg" },
  { name: "Registered Labour Hire Provider", image: "/labour-hire-provider.svg" },
]
  ...
  return (
    <section className="bg-background py-16 lg:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-14 flex max-w-2xl flex-wrap items-center justify-center gap-6 border-t border-border pt-8">
          {accreditations.map((item, index) => (
            <div
              key={index}
              className="flex h-24 w-28 items-center justify-center rounded-lg border border-border bg-card px-3"
            >
              <Image
                src={item.image}
                alt={item.name}
                width={100}
                height={120}
                className="max-h-20 w-auto object-contain opacity-80 transition-opacity hover:opacity-100"
              />
            </div>
          ))}
        </div>
